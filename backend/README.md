...
- 🛡️ Persisting and analyzing anti-cheat violation logs
- 📊 Serving analytical data for tutor and admin dashboards
- ✉️ Email notifications (verified via [Mailtrap](https://mailtrap.io), production via **Google Gmail SMTP**)
...
| #️⃣ **C#** | Language (Nullable reference types enabled) |
| 🏛️ **Clean Architecture** | Vertical Slice Hybrid approach |
...
| ✉️ **MailKit / MimeKit** | Email delivery (Mailtrap for testing, Gmail SMTP for production) |
...
2. 🗄️ **SQL Server**: The database where all users, courses, and exam data will be stored.
...
| F17 | 🛡️ **Anti-Cheating Engine** | Browser-native exam integrity system |
...
| 🛡️ Violations | `/api/violations` | batch report, per-attempt, per-exam |
...
| 🤖 AI | `/api/ai` | chat |
| 🖼️ Files | `/uploads` | profile pictures, course materials |
